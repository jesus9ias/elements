...
| 2026-07-27 | **Corrects the row above.** The `es` override for Zn's `discoverer` is `"Andreas Sigismund Marggraf"`, not `"Andreas Marggraf"` — both languages carry the identical string. The row above's "spelled as each language's own Wikipedia article writes it" therefore describes `en` only | Developer's own edit to `data/curated/overrides/es/Zn.json` before committing (`f220754`). The `es` article's shorter "Andreas Marggraf" names the same chemist, so nothing about the attribution changes; carrying one identical string in both configs removes a per-language spelling that would otherwise have to be remembered and kept in sync for no informational gain. Recorded rather than edited into the row above, per the log's "add a dated row, don't rewrite history" convention |
| 2026-07-27 | `merge.ts`'s `--element=<symbol>` run now splices the merged record into the existing `src/config/elements.{lang}.json` (`readExistingElements()` + object spread in `main()`) instead of writing the scoped slice as the whole file. `common.json` is rebuilt from the spliced result, not the slice. A scoped run with no existing config is a hard error naming the full run, not a silent partial write | Found while fixing Zn: the header comment advertised `--element=Cr` as supported usage, but `mergeLanguage()` skips every non-matching symbol and `main()` then wrote that single-record object over the aggregate config — so `--element=Zn` would have deleted the other 117 elements and reduced `common.json` to one entry. The full Zn merge was run unscoped for exactly this reason. Note this bug is specific to `merge.ts`: `fetch-elements.ts` takes the same flag harmlessly because it writes one file per element, so nothing there is aggregate to clobber. Verified: a scoped run leaves all three files byte-identical to a full run (md5), keeps 118 entries with keys still in ascending numeric order (integer-like JS object keys iterate ascending regardless of insertion order, so the spread cannot scramble them — checked, not assumed), and a mutation test — sentinel written into Zn's `discoverer` plus a canary name on element 77 — confirms the scoped run restores Zn from source while carrying the canary through untouched. The missing-config guard exits 1 before writing anything. 21/21 tests pass. **`--element` deliberately does NOT refresh the other 117** — they are carried through as-is, stale prose included; that is what makes the flag useful for re-applying one override, and why any change to shared inputs (`categories.ts`, `isotopes.json`, `electron-configurations.json`) still needs a full run | 
| 2026-07-28 | **Corrects the 2026-07-23 `CONFIRMED_ABSENT` row, and the doc comments written from it.** That row justified the marker with "Wikidata's own P61 statement is an explicit 'unknown value', not merely absent" and applied that description to all of Fe/Au/Ag/Cu. Per `data/raw/`, only **Fe and Au** carry a genid; **Ag and Cu have no P61 statement at all** — the same day's `resolveDiscoverer()` row already named the genid set correctly as Fe/Au/Zn. The `CONFIRMED_ABSENT` comment (`src/constants/pipeline.ts`) and the data-conventions bullet in `frontend/claude.md` are rewritten to state the real criterion: **the marker is licensed by the curated reading of the source articles, never by the shape — or the absence — of the Wikidata statement** | Found while deciding Sn, whose case rests on exactly the half of the old wording that was wrong. An absent statement is not evidence of anything: it means only that nobody filled it in, so accepting it as grounds would license the marker for any element Wikidata has not gotten to — the precise "not yet looked at" case the marker exists to distinguish. Zn remains the standing counter-example in the other direction: the strongest possible Wikidata signal (both fields "unknown value") and the correct answer was still a real curated value. The 2026-07-23 row is left intact per the log's "add a dated row, don't rewrite history" convention |
| 2026-07-28 | Sn's `discoveryDate` **and** `discoverer` are both marked `CONFIRMED_ABSENT` (`data/curated/overrides/{en,es}/Sn.json`) — the first element to carry the marker on `discoveryDate`. Sb/Bi/Hg/Pb stay flagged and are still open, to be reviewed one element at a time | Developer's decision: tin has no precise discoverer or isolator, and — unlike copper — it was not used on its own or understood as a distinct substance in prehistory; it enters the record only through bronze. That same reasoning rules out a curated year, so the field is asserted absent rather than filled: `en.wikipedia` dates tin extraction to "around 3000 BC" and `es.wikipedia` to "alrededor del 4000 a. C.", a millennium apart, and both date **the alloy, not the element** — the identical objection that rejected 1000 BCE for Zn on 2026-07-27. The only element-level statement available, en's "After 600 BC, pure metallic tin was produced", is a bound, not a discovery year. The resulting asymmetry is deliberate and source-driven rather than factual: Cu and Ag render a BCE year only because Wikidata happens to hold P575 for them, while Sn (no P575, no P61) renders `—` on both fields. Verified: full unscoped merge, diff touches exactly one line per language (Sn's `needsReview` true→false) and nothing in `common.json`, both fields ship as real `null`, `grep -r "confirmed-absent" src/config/ data/generated/` is empty, a re-run reproduces all three config files byte-identical (md5), 21/21 tests pass. Flagged count is now 32 (en) / 33 (es); the one-element gap is pre-existing and unrelated — Fl is flagged in `es` only |
| 2026-07-28 | **Antoine Lavoisier was considered and rejected as Sn's `discoverer`**; the row above stands unchanged. No data or code changed. Standing rule while the question is open: whatever criterion is in force is applied **uniformly** across the antiquity elements — a single element is never re-decided on its own | Developer found third-party mentions crediting Lavoisier with officially isolating tin. Checked against the pipeline's own inputs: `grep -i "lavoisier"` returns **0** hits in `data/raw/{en,es}/Sn.json` and both `data/generated/` files, as do "1789", "isolat*" and "aisló" — so writing it would be model knowledge, not extraction. Wikidata is decisive and internally consistent here: the same SPARQL run that returned P61 = Lavoisier for C (1789) and S (1777) returned nothing at all for Sn, so the omission is deliberate, not an unfilled field. Substantively, nobody isolated tin — it was smelted from cassiterite for millennia, and en's own text says "After 600 BC, pure metallic tin was produced"; the 1789 act was listing tin among the simple substances in the *Traité élémentaire de chimie*, i.e. classification, which is why element tables that fill "discoverer" with "first recognized it as an element" all land on Lavoisier for every antiquity metal. Accepting it for Sn would therefore mean accepting it for Cu/Fe/Ag/Au/Pb/Hg too and reverting all five `CONFIRMED_ABSENT` markers — a policy change, not a per-element fix. **Known inconsistency, inherited from Wikidata and not chosen here**: C and S ship "Antoine Lavoisier" while Fe/Au/Ag/Cu ship `—`, and the only thing separating those cases is whether P61 was filled in. Developer's decision: leave it, keep the criterion uniform, and revisit with a more precise definition of "discoverer" later — that definition is the open item, in either direction |
