...
- `needsReview: true` marks a record with a genuinely missing required field. It is the designed visible-gap mechanism, **not** an error — do not "fix" it by inventing values or by relaxing the required-field list. The one sanctioned escape hatch is `CONFIRMED_ABSENT` (`src/constants/pipeline.ts`): a curated override can hold this marker instead of `null` to assert "checked, genuinely absent" rather than "not yet looked at" (e.g. Fe/Au/Ag/Cu/Sn's `discoverer`, plus Sn's `discoveryDate`). `merge.ts` excludes a field holding it from `needsReview`, then writes real `null` in its place — the marker never reaches the shipped config or the UI, and the value shown to the user is unchanged (`—`, same as any other missing field). Adding this marker to an element is a curated, per-field assertion — it is not a way to relax the required-field list globally.

  **What licenses the marker is the source article, never the shape of the Wikidata statement.** Of the five elements carrying it, only Fe and Au have an explicit P61 "unknown value"; Ag, Cu and Sn have no P61 statement at all, and Sn has no P575 either — an absent statement means only that nobody filled it in, so it is never grounds on its own. Zn is the counter-example that makes this concrete: *both* its fields came back as "unknown value" genids, the strongest possible Wikidata signal, and the correct answer was still a real curated value (1746 / Marggraf) because the articles do credit an isolator. Read the extracts in `data/raw/{lang}/`, and treat each element separately — Sb/Bi/Hg/Pb are still open for exactly this reason.
